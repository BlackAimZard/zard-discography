...
const DATA_PATH = path.join(__dirname, "dvd_data.js");
const TEMPLATE_PATH = path.join(__dirname, "dvd_template.html");
const OUTPUT_PATH = path.join(__dirname, "dvd.html");
const PLACEHOLDER = "{{DVD_ROWS}}";
...
class StrictDataParser {
    ...
    parse() {
        ...
        if (declaration.value !== "DVD_DATA") {
            this.fail(
                declaration.start,
                'The only allowed declaration is "const DVD_DATA"'
            );
        }

        this.skipWhitespace();
        this.expectCharacter("=");
        const data = this.parseValue("DVD_DATA");

        if (!Array.isArray(data)) {
            this.fail(
                this.index,
                "DVD_DATA must be an array literal"
            );
        }
        ...
        if (this.index !== this.source.length) {
            this.fail(
                this.index,
                "No code, export, comment, or other content is allowed after DVD_DATA"
            );
        }
    ...
    validateSourceHygiene() {
        ...
        if (unicodeIssue) {
            this.fail(
                unicodeIssue.index,
                `dvd_data.js ${unicodeIssue.message}`
            );
        }
        ...
        if (
            ...
            this.fail(
                0,
                "dvd_data.js must use one consistent line-ending style"
            );
        }
        ...
        if (
            ...
            this.fail(
                this.source.length,
                "dvd_data.js must end with one line break"
            );
        }
    }
    ...
    parseFieldGuide() {
        ...
        if (!this.source.startsWith(fieldGuideSource)) {
            this.fail(
                0,
                `dvd_data.js must begin with the exact fixed ${RECORD_FIELDS.length}-field guide`
            );
        }
...
function getValidationExample(pathLabel) {
    ...
    if (/^DVD_DATA\[[0-9]+\]$/.test(pathLabel)) {
        return (
    ...
    if (/\.cover$/.test(pathLabel)) {
        return (
            'cover: { src: "assets/disc_photo/dvd/dvd01.jpg" } ' +
            "or cover: null."
        );
    }
    ...
    const examples = {
        ...
        src: 'src: "assets/disc_photo/dvd/dvd01.jpg".',
        href: 'href: "https://example.com".',
        category: 'category: "Live DVD" or category: null.'
    };
...
function validateCover(
    ...
    if (Object.hasOwn(cover, "src")) {
        ...
        if (
            ...
            !/^assets\/disc_photo\/dvd\/[^/]+\.jpg$/.test(cover.src)
        ) {
            errors.add(
                `${label}.src must use the assets/disc_photo/dvd/ folder and a .jpg filename.`,
                "FIELD VALUES",
                {
                    example: 'src: "assets/disc_photo/dvd/dvd01.jpg".',
                    path: `${pathLabel}.src`
                }
            );
        }
...
function validateRecord(
    ...
    const recordPath = `DVD_DATA[${index}]`;
    const recordLabel = (
        isObject(item) &&
    ...
    if (Object.hasOwn(item, "id")) {
        if (
            ...
            const idPattern = /^dvd[A-Za-z0-9]+$/;

            if (!idPattern.test(item.id)) {
                errors.add(
                    `${recordLabel}.id must start with "dvd" and contain ASCII letters or numbers only.`,
                    "FIELD VALUES",
                    {
                        path: `${recordPath}.id`
                    }
...
function formatDataSource(
    ...
    const source = [
        ...
        "",
        `const DVD_DATA = ${formatDataValue(
            data,
            0,
            eol,
            indentationUnit
...
function parseAndValidateData() {
    ...
    if (
        ...
        errors.add(
            "DVD_DATA must contain at least one record.",
            "DATA STRUCTURE",
            {
                example: "const DVD_DATA = [ { ... } ];",
                path: "DVD_DATA"
            }
        );
    } else {
    ...
    if (errors.items.length === 0) {
        ...
        if (
            ...
            errors.add(
                "dvd_data.js formatting is not canonical. Run node build_dvd.js --format after fixing all data errors.",
                "DATA FORMAT",
                {
...
function renderOutput(template, data) {
    ...
    if (placeholderCount !== 1) {
        throw new Error(
            `dvd_template.html must contain exactly one ${PLACEHOLDER}; found ${placeholderCount}.`
        );
    }
...
function getBuildOptions() {
    ...
    throw new Error(
        "Unknown command. Use node build_dvd.js or node build_dvd.js --format."
    );
}

function buildDVDHTML() {
    ...
    try {
        ...
        issues.add(
            "DATA SYNTAX",
            (
                `dvd_data.js — ${error.message}\n` +
                "   Correct outer form: const DVD_DATA = [ ... ];"
            )
        );
    }
    ...
    try {
        ...
        issues.add(
            "TEMPLATE HTML",
            `dvd_template.html — ${error.message}`
        );
    }
    ...
    console.log(
        `Built dvd.html from ${dataResult.data.length} DVD records.`
    );
}
