...
const DATA_PATH = path.join(__dirname, "album_data.js");
const TEMPLATE_PATH = path.join(__dirname, "album_template.html");
const OUTPUT_PATH = path.join(__dirname, "album.html");
const PLACEHOLDER = "{{ALBUM_ROWS}}";
...
class StrictDataParser {
    ...
    parse() {
        ...
        if (declaration.value !== "ALBUM_DATA") {
            this.fail(
                declaration.start,
                'The only allowed declaration is "const ALBUM_DATA"'
            );
        }

        this.skipWhitespace();
        this.expectCharacter("=");
        const data = this.parseValue("ALBUM_DATA");

        if (!Array.isArray(data)) {
            this.fail(
                this.index,
                "ALBUM_DATA must be an array literal"
            );
        }
        ...
        if (this.index !== this.source.length) {
            this.fail(
                this.index,
                "No code, export, comment, or other content is allowed after ALBUM_DATA"
            );
        }
    ...
    validateSourceHygiene() {
        ...
        if (unicodeIssue) {
            this.fail(
                unicodeIssue.index,
                `album_data.js ${unicodeIssue.message}`
            );
        }
        ...
        if (
            ...
            this.fail(
                0,
                "album_data.js must use one consistent line-ending style"
            );
        }
        ...
        if (
            ...
            this.fail(
                this.source.length,
                "album_data.js must end with one line break"
            );
        }
    }
    ...
    parseFieldGuide() {
        ...
        if (!this.source.startsWith(fieldGuideSource)) {
            this.fail(
                0,
                `album_data.js must begin with the exact fixed ${RECORD_FIELDS.length}-field guide`
            );
        }
...
function getValidationExample(pathLabel) {
    ...
    if (/^ALBUM_DATA\[[0-9]+\]$/.test(pathLabel)) {
        return (
    ...
    const examples = {
        type: 'type: "album" or type: "promotional".',
        id: 'id: "al01", id: "BestAlbum01", or id: "pr01".',
...
class ValidationErrors {
    ...
    add(
        ...
        const lines = [
            location === null
                ? `album_data.js — ${message}`
                : `album_data.js, ${location} — ${message}`
        ];
...
function validateTrackTieup(
    ...
    if (!Array.isArray(value) || value.length === 0) {
        errors.add(
            `${label} must be a non-empty array of strings or null.`,
            "FIELD VALUES",
            {
                example: 'Use tieup: [ "Theme Song" ] or tieup: null; do not use an empty array.',
                path: pathLabel
            }
        );
        return;
    ...
    value.forEach((text, index) => {
        ...
        const isValid = validateCleanString(
            text,
            itemLabel,
            errors,
            itemPath
        ...
        if (firstIndexes.has(text)) {
            errors.add(
                `${itemLabel} duplicates ${label}[${firstIndexes.get(text)}].`,
                "FIELD VALUES",
                {
                    example: "Keep each tie-up text once within the same track.",
                    path: itemPath
                }
            );
            return;
...
function validateTracks(
    ...
    if (hasNullDisc && hasNumberedDisc) {
        errors.add(
            ...
            "FIELD VALUES",
            {
                example: "Use disc: null for every track on a single-disc album, or disc: 1, disc: 2, ... for every track on a multi-disc album.",
                path: `${recordPath}.tracks`
            }
        );
        return;
...
function validateRecord(
    ...
    const recordPath = `ALBUM_DATA[${index}]`;
    const recordLabel = (
        isObject(item) &&
    ...
    if (
        ...
        errors.add(
            `${recordLabel}.type must be "album" or "promotional".`,
            "FIELD VALUES",
            {
    ...
    if (Object.hasOwn(item, "id")) {
        if (
            ...
            if (!idPattern.test(item.id)) {
                errors.add(
                    item.type === "promotional"
                        ? `${recordLabel}.id must use the lowercase pr prefix followed by at least one ASCII letter or number.`
...
function formatDataSource(
    ...
    const source = [
        ...
        "",
        `const ALBUM_DATA = ${formatDataValue(
            data,
            0,
            eol,
            indentationUnit
...
function parseAndValidateData() {
    ...
    if (
        ...
        errors.add(
            "ALBUM_DATA must contain at least one record.",
            "DATA STRUCTURE",
            {
                example: "const ALBUM_DATA = [ { ... } ];",
                path: "ALBUM_DATA"
            }
        );
    } else {
    ...
    if (errors.items.length === 0) {
        ...
        if (
            ...
            errors.add(
                "album_data.js formatting is not canonical. Run node build_album.js --format after fixing all data errors.",
                "DATA FORMAT",
                {
...
function createTieupHTML(tracks) {
    if (tracks === null) {
        return "";
    }
    ...
    return sortedTracks
        .flatMap(track => {
            ...
            const trackReference = multiDisc
                ? `(Disc ${track.disc} / #${track.track})`
                : `(#${track.track})`;

            return track.tieup.map(text => {
                return (
                    `${escapeHTML(text)} ` +
                    trackReference
                );
...
function createRecordHTML(item, eol) {
    const number = /^(?:al|pr)[A-Za-z0-9]+$/.test(item.id)
        ? item.id.slice(2)
        : item.id;
    const recordName = item.type === "album"
        ? "Album"
        : "Promotional Album";
    ...
    const tieup = createTieupHTML(item.tracks);
...
function renderOutput(template, data) {
    ...
    if (placeholderCount !== 1) {
        throw new Error(
            `album_template.html must contain exactly one ${PLACEHOLDER}; found ${placeholderCount}.`
        );
    }
...
function getBuildOptions() {
    ...
    throw new Error(
        "Unknown command. Use node build_album.js or node build_album.js --format."
    );
}
...
function buildAlbumHTML() {
    ...
    try {
        ...
        issues.add(
            "DATA SYNTAX",
            (
                `album_data.js — ${error.message}\n` +
                "   Correct outer form: const ALBUM_DATA = [ ... ];"
            )
        );
    }
    ...
    try {
        ...
        issues.add(
            "TEMPLATE HTML",
            `album_template.html — ${error.message}`
        );
    }
    ...
    const albumCount = dataResult.data.filter(item => {
    ...
    console.log(
        `Built album.html from ${albumCount} albums and ${promotionalCount} promotional releases.`
    );
}
