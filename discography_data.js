const TIMELINE_TYPE_CONFIG = {
    single: {
        page: "single.html",
        folder: "sg",
        thumbFolder: "sg-node-thumb"
    },
    album: {
        page: "album.html",
        folder: "al",
        thumbFolder: "al-node-thumb"
    }
};
...
function createTimelineNodeHTML(item) {
    ...
    const cover = escapeTimelineHTML(
        item.cover || `assets/disc_photo/${config.folder}/${item.id}.jpg`
    );
    const thumb = escapeTimelineHTML(
        item.thumb ||
        `assets/disc_photo/${config.folder}/${config.thumbFolder}/${item.id}.jpg`
    );
