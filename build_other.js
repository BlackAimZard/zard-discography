...
const DATA_PATH = path.join(__dirname, "other_data.js");
const TEMPLATE_PATH = path.join(__dirname, "other_template.html");
const OUTPUT_PATH = path.join(__dirname, "other.html");
const PLACEHOLDER = "{{OTHER_ROWS}}";
...
class StrictDataParser {
    ...
    parse() {
        ...
        if (declaration.value !== "OTHER_DATA") {
            this.fail(
                declaration.start,
                'The only allowed declaration is "const OTHER_DATA"'
            );
        }

        this.skipWhitespace();
        this.expectCharacter("=");
        const data = this.parseValue("OTHER_DATA");

        if (!Array.isArray(data)) {
            this.fail(
                this.index,
                "OTHER_DATA must be an array literal"
            );
        }
        ...
        if (this.index !== this.source.length) {
            this.fail(
                this.index,
                "No code, export, comment, or other content is allowed after OTHER_DATA"
            );
        }
    ...
    validateSourceHygiene() {
        ...
        if (unicodeIssue) {
            this.fail(
                unicodeIssue.index,
                `other_data.js ${unicodeIssue.message}`
            );
        }
        ...
        if (
            ...
            this.fail(
                0,
                "other_data.js must use one consistent line-ending style"
            );
        }
        ...
        if (
            ...
            this.fail(
                this.source.length,
                "other_data.js must end with one line break"
            );
        }
    }
    ...
    parseFieldGuide() {
        ...
        if (!this.source.startsWith(fieldGuideSource)) {
            this.fail(
                0,
                `other_data.js must begin with the exact fixed ${RECORD_FIELDS.length}-field guide`
            );
        }
...
function getValidationExample(pathLabel) {
    ...
    if (/^OTHER_DATA\[[0-9]+\]$/.test(pathLabel)) {
        return (
    ...
    if (/\.cover$/.test(pathLabel)) {
        return (
            'cover: { src: "assets/disc_photo/oth/oth01.jpg" } ' +
            "or cover: null."
        );
    }

    if (/\.tracks$/.test(pathLabel)) {
        return "tracks: [ { disc: null, track: 1, title: \"Song\", japaneseTitle: null, credit: null, karaoke: false, tieup: null } ] or tracks: null.";
    }

    if (/\.tracks\[[0-9]+\]$/.test(pathLabel)) {
        return '{ disc: null, track: 1, title: "Song", japaneseTitle: null, credit: null, karaoke: false, tieup: null }';
    }
    ...
    const examples = {
        type: 'type: "other".',
        id: 'id: "SoundBlend01", id: "RemixAlbum01", or id: "oth01".',
        order: "order: null.",
        title: 'title: "Tokimeki Memorial SOUND BLEND featuring ZARD".',
        ...
        src: 'src: "assets/disc_photo/oth/oth01.jpg".',
        disc: "disc: null for a single-disc Other release, or disc: 1, disc: 2, ... for a multi-disc Other release.",
        ...
        category: 'category: "Other / Collab" or category: null.'
    };
...
class ValidationErrors {
    ...
    add(
        ...
        const lines = [
            location === null
                ? `other_data.js — ${message}`
                : `other_data.js, ${location} — ${message}`
        ];
...
function validateCover(
    ...
    if (Object.hasOwn(cover, "src")) {
        ...
        if (
            ...
            !/^assets\/disc_photo\/oth\/[^/]+\.jpg$/.test(cover.src)
        ) {
            errors.add(
                `${label}.src must use the assets/disc_photo/oth/ folder and a .jpg filename.`,
                "FIELD VALUES",
                {
                    example: 'src: "assets/disc_photo/oth/oth01.jpg".',
                    path: `${pathLabel}.src`
                }
            );
        }
...
function validateTrackTieup(
    ...
    errors.add(
        `${label} must be null because Other records do not render a Tie-up column.`,
        "FIELD VALUES",
        {
            example: "tieup: null.",
            path: pathLabel
...
function validateTracks(
    ...
    tracks.forEach((track, index) => {
        ...
        if (Object.hasOwn(track, "disc")) {
            if (
                track.disc !== null &&
                (
                    !Number.isInteger(track.disc) ||
                    track.disc < 1
                )
            ) {
                errors.add(
                    `${label}.disc must be null or a positive integer.`,
                    "FIELD VALUES",
                    {
                        path: `${pathLabel}.disc`
                    }
        ...
        if (Object.hasOwn(track, "track")) {
            if (
                !Number.isInteger(track.track) ||
                track.track < 1
            ) {
                errors.add(
                    `${label}.track must be a positive integer.`,
                    "FIELD VALUES",
                    {
                        path: `${pathLabel}.track`
                    }
                );
            }
        }
    ...
    const hasNullDisc = tracks.some(track => track.disc === null);
    const hasNumberedDisc = tracks.some(track => {
        return Number.isInteger(track.disc) && track.disc >= 1;
    });
    const hasInvalidSequenceValue = tracks.some(track => {
        return (
            !(
                track.disc === null ||
                (
    ...
    if (hasNullDisc && hasNumberedDisc) {
        errors.add(
            `${recordLabel}.tracks must not mix disc: null with numbered discs.`,
            "FIELD VALUES",
            {
                example: "Use disc: null for every track on a single-disc Other release, or disc: 1, disc: 2, ... for every track on a multi-disc Other release.",
                path: `${recordPath}.tracks`
            }
        );
        return;
    ...
    if (hasInvalidSequenceValue) {
        return;
    }

    if (hasNullDisc) {
        tracks.forEach((track, index) => {
            const expectedTrack = index + 1;

            if (track.track !== expectedTrack) {
                errors.add(
                    `${recordLabel}.tracks[${index}].track must be ${expectedTrack}; single-disc track numbers must start at 1 and remain continuous in source order.`,
                    "FIELD VALUES",
                    {
                        path: `${recordPath}.tracks[${index}].track`
                    }
                );
            }
        });
    ...
    tracks.forEach((track, index) => {
        if (index === 0) {
            if (track.disc !== 1 || track.track !== 1) {
                errors.add(
                    `${recordLabel}.tracks[0] must start with disc: 1 and track: 1.`,
                    "FIELD VALUES",
                    {
                        path: `${recordPath}.tracks[0]`
                    }
                );
            }
            return;
        ...
        const continuesDisc = (
            track.disc === previous.disc &&
            track.track === previous.track + 1
        );
        const startsNextDisc = (
            track.disc === previous.disc + 1 &&
            track.track === 1
        );

        if (!continuesDisc && !startsNextDisc) {
            errors.add(
                `${recordLabel}.tracks[${index}] must continue Disc ${previous.disc} at Track ${previous.track + 1}, or start Disc ${previous.disc + 1} at Track 1.`,
                "FIELD VALUES",
                {
                    path: `${recordPath}.tracks[${index}]`
                }
            );
        }
    });
...
function validateRecord(
    ...
    const recordPath = `OTHER_DATA[${index}]`;
    const recordLabel = (
        isObject(item) &&
    ...
    if (
        ...
        errors.add(
            `${recordLabel}.type must be "other".`,
            "FIELD VALUES",
            {
    ...
    if (Object.hasOwn(item, "id")) {
        if (
            ...
            if (!idPattern.test(item.id)) {
                errors.add(
                    `${recordLabel}.id must start with an ASCII letter and contain ASCII letters or numbers only.`,
                    "FIELD VALUES",
                    {
                        path: `${recordPath}.id`
                    }
    ...
    if (Object.hasOwn(item, "order")) {
        if (item.order !== null) {
            errors.add(
                `${recordLabel}.order must be null so the No. column remains blank.`,
                "FIELD VALUES",
                {
                    example: "order: null.",
                    path: `${recordPath}.order`
                }
            );
        }
    }
...
function formatDataSource(
    ...
    const source = [
        ...
        "",
        `const OTHER_DATA = ${formatDataValue(
            data,
            0,
            eol,
            indentationUnit
...
function parseAndValidateData() {
    ...
    if (
        ...
        errors.add(
            "OTHER_DATA must contain at least one record.",
            "DATA STRUCTURE",
            {
                example: "const OTHER_DATA = [ { ... } ];",
                path: "OTHER_DATA"
            }
        );
    } else {
    ...
    if (errors.items.length === 0) {
        ...
        if (
            ...
            errors.add(
                "other_data.js formatting is not canonical. Run node build_other.js --format after fixing all data errors.",
                "DATA FORMAT",
                {
...
function compareTracks(left, right) {
    const leftDisc = left.disc === null
        ? 1
        : left.disc;
    const rightDisc = right.disc === null
        ? 1
        : right.disc;
...
function createTracksHTML(tracks, eol) {
    ...
    const multiDisc = hasMultipleDiscs(sortedTracks);

    return sortedTracks
        .map((track, index) => {
            const previous = sortedTracks[index - 1];
            const discHeading = (
                multiDisc &&
                (
                    previous === undefined ||
                    previous.disc !== track.disc
                )
            )
                ? (
                    `<b class="disc-heading">● Disc ` +
                    `${String(track.disc).padStart(2, "0")}</b>`
                )
                : "";
...
function renderOutput(template, data) {
    ...
    if (placeholderCount !== 1) {
        throw new Error(
            `other_template.html must contain exactly one ${PLACEHOLDER}; found ${placeholderCount}.`
        );
    }
...
function validateRecordTableLayout(
    ...
    data.forEach(item => {
        ...
        if (firstRow.cells.length !== 5) {
            throw new HtmlSyntaxError(
                output,
                firstRow.index,
                `Record "${item.id}" must contain five cells in its first row`
            );
        }
...
function getBuildOptions() {
    ...
    throw new Error(
        "Unknown command. Use node build_other.js or node build_other.js --format."
    );
}
...
function buildOtherHTML() {
    ...
    try {
        ...
        issues.add(
            "DATA SYNTAX",
            (
                `other_data.js — ${error.message}\n` +
                "   Correct outer form: const OTHER_DATA = [ ... ];"
            )
        );
    }
    ...
    try {
        ...
        issues.add(
            "TEMPLATE HTML",
            `other_template.html — ${error.message}`
        );
    }
    ...
    console.log(
        `Built other.html from ${dataResult.data.length} Other records.`
    );
}
