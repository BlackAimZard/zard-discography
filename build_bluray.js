...
const DATA_PATH = path.join(__dirname, "bluray_data.js");
const TEMPLATE_PATH = path.join(__dirname, "bluray_template.html");
const OUTPUT_PATH = path.join(__dirname, "bluray.html");
const PLACEHOLDER = "{{BLURAY_ROWS}}";
...
class StrictDataParser {
    ...
    parse() {
        ...
        if (declaration.value !== "BLURAY_DATA") {
            this.fail(
                declaration.start,
                'The only allowed declaration is "const BLURAY_DATA"'
            );
        }

        this.skipWhitespace();
        this.expectCharacter("=");
        const data = this.parseValue("BLURAY_DATA");

        if (!Array.isArray(data)) {
            this.fail(
                this.index,
                "BLURAY_DATA must be an array literal"
            );
        }
        ...
        if (this.index !== this.source.length) {
            this.fail(
                this.index,
                "No code, export, comment, or other content is allowed after BLURAY_DATA"
            );
        }
    ...
    validateSourceHygiene() {
        ...
        if (unicodeIssue) {
            this.fail(
                unicodeIssue.index,
                `bluray_data.js ${unicodeIssue.message}`
            );
        }
        ...
        if (
            ...
            this.fail(
                0,
                "bluray_data.js must use one consistent line-ending style"
            );
        }
        ...
        if (
            ...
            this.fail(
                this.source.length,
                "bluray_data.js must end with one line break"
            );
        }
    }
    ...
    parseFieldGuide() {
        ...
        if (!this.source.startsWith(fieldGuideSource)) {
            this.fail(
                0,
                `bluray_data.js must begin with the exact fixed ${RECORD_FIELDS.length}-field guide`
            );
        }
...
function getValidationExample(pathLabel) {
    ...
    if (/^BLURAY_DATA\[[0-9]+\]$/.test(pathLabel)) {
        return (
    ...
    if (/\.cover$/.test(pathLabel)) {
        return (
            'cover: { src: "assets/disc_photo/bd/bd01.jpg" } ' +
            "or cover: null."
        );
    }

    if (/\.content$/.test(pathLabel)) {
        return 'content: [ "Main program", "Bonus footage" ] or content: null.';
    }
    ...
    const examples = {
        type: 'type: "blu-ray".',
        id: 'id: "bd01", id: "bd02", or id: "bdSpecial01".',
        order: "order: null.",
        title: 'title: "What a beautiful moment".',
        japaneseTitle: 'japaneseTitle: "負けないで" or japaneseTitle: null.',
        format: 'format: "Blu-ray" or format: null.',
        label: 'label: "B-Gram RECORDS" or label: null.',
        catalog: 'catalog: "JBBJ-5001" or catalog: null.',
        ...
        src: 'src: "assets/disc_photo/bd/bd01.jpg".',
        href: 'href: "https://example.com".',
        category: 'category: "Live Blu-ray" or category: null.'
    };
...
class ValidationErrors {
    ...
    add(
        ...
        const lines = [
            location === null
                ? `bluray_data.js — ${message}`
                : `bluray_data.js, ${location} — ${message}`
        ];
...
function validateCover(
    ...
    if (Object.hasOwn(cover, "src")) {
        ...
        if (
            ...
            !/^assets\/disc_photo\/bd\/[^/]+\.jpg$/.test(cover.src)
        ) {
            errors.add(
                `${label}.src must use the assets/disc_photo/bd/ folder and a .jpg filename.`,
                "FIELD VALUES",
                {
                    example: 'src: "assets/disc_photo/bd/bd01.jpg".',
                    path: `${pathLabel}.src`
                }
            );
        }
...
function validateContent(
    ...
    if (!Array.isArray(content) || content.length === 0) {
        errors.add(
            `${recordLabel}.content must be a non-empty array of strings or null.`,
            "FIELD VALUES",
            {
                example: 'content: [ "Main program", "Bonus footage" ] or content: null.',
                path: `${recordPath}.content`
            }
        );
        return;
    }
    ...
    content.forEach((line, index) => {
        const pathLabel = `${recordPath}.content[${index}]`;

        if (
            !validateCleanString(
                line,
                `${recordLabel}.content[${index}]`,
                errors,
                pathLabel
            )
        ) {
        ...
        if (usedLines.has(line)) {
            errors.add(
                `${recordLabel}.content contains the duplicate line "${line}".`,
                "DUPLICATE CONTENT",
                {
                    example: "Each content line must be unique within one record.",
                    path: pathLabel
                }
            );
        } else {
...
function validateRecord(
    ...
    const recordPath = `BLURAY_DATA[${index}]`;
    const recordLabel = (
        isObject(item) &&
    ...
    if (
        ...
        errors.add(
            `${recordLabel}.type must be "blu-ray".`,
            "FIELD VALUES",
            {
    ...
    if (Object.hasOwn(item, "id")) {
        if (
            ...
            const idPattern = /^bd[A-Za-z0-9]+$/;

            if (!idPattern.test(item.id)) {
                errors.add(
                    `${recordLabel}.id must start with "bd" and contain ASCII letters or numbers only.`,
                    "FIELD VALUES",
                    {
                        path: `${recordPath}.id`
                    }
...
function formatDataSource(
    ...
    const source = [
        ...
        "",
        `const BLURAY_DATA = ${formatDataValue(
            data,
            0,
            eol,
            indentationUnit
...
function parseAndValidateData() {
    ...
    if (
        ...
        errors.add(
            "BLURAY_DATA must contain at least one record.",
            "DATA STRUCTURE",
            {
                example: "const BLURAY_DATA = [ { ... } ];",
                path: "BLURAY_DATA"
            }
        );
    } else {
    ...
    if (errors.items.length === 0) {
        ...
        if (
            ...
            errors.add(
                "bluray_data.js formatting is not canonical. Run node build_bluray.js --format after fixing all data errors.",
                "DATA FORMAT",
                {
...
function createRecordHTML(item, eol) {
    ...
    const content = createContentHTML(item.content);
...
function renderOutput(template, data) {
    ...
    if (placeholderCount !== 1) {
        throw new Error(
            `bluray_template.html must contain exactly one ${PLACEHOLDER}; found ${placeholderCount}.`
        );
    }
...
function getBuildOptions() {
    ...
    throw new Error(
        "Unknown command. Use node build_bluray.js or node build_bluray.js --format."
    );
}

function buildBlurayHTML() {
    ...
    try {
        ...
        issues.add(
            "DATA SYNTAX",
            (
                `bluray_data.js — ${error.message}\n` +
                "   Correct outer form: const BLURAY_DATA = [ ... ];"
            )
        );
    }
    ...
    try {
        ...
        issues.add(
            "TEMPLATE HTML",
            `bluray_template.html — ${error.message}`
        );
    }
    ...
    console.log(
        `Built bluray.html from ${dataResult.data.length} Blu-ray records.`
    );
}
